...
Estos scripts están implementados y probados en python 2.7.
...
\section{Formato de los datos}
   ...
   \begin{itemize}
       ...
       \begin{itemize}
           ...
           extraídos directamente de la señal \underline{original} con una 
           longitud 
           ...
           detectados 
           extraídos de la señal \underline{filtrada} con una longitud 
           idéntica a la usada en la ventana del algoritmo de identificación. 
           Esta tabla tiene los mismos atributos que \texttt{Time}.
...
\begin{itemize}
    ...
    \item \texttt{peaks\_smooth}: Diccionario con los parámetros del suavizado 
    que se realiza a la señal y reubicación de la basal: \texttt{pcasmooth}, 
...
\begin{enumerate}
    ...
    creará un script equivalente). Se creará el fichero en el directorio del 
    experimento y se crearán las  carpetas para cada fichero/tramo del 
    experimento y se copiaran la señales en las posiciones indicadas en el 
    parámetro \texttt{abfsensors} en las tablas \texttt{Raw}.
...
número de clusters definido para cada sensor del experimento en el parámetro 
\texttt{clusters}.
...
\begin{itemize}
    ...
    de los clusters. Estos centroides están ordenados de menor a mayor según la 
    altura del máximo de cada centroide.
    
    \item \texttt{PeaksClusteringHisto.py}, que aplica el algoritmo de cluster 
    y genera un gráfico con los centroides de los clusters del primer fichero 
    de datos (es el que usa como referencia para el resto) y un gráfico también 
    con los centroides, pero incluyendo un histograma para cada centroide de la 
    proporción de picos en cada tramo del experimento.
\end{itemize}
